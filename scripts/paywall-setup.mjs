...
// credential configured" = no key on file yet (os.ibl.ai → Integrations →
// Data Sources → Add Data Source → Stripe).
 ...
 *   access: string, amount?: number | null,
 * }} args
 * @param {typeof fetch} [fetchImpl]
 */
export async function setupPaywall(
  { dmUrl, platform, token, slug, appName = "", access, amount = null },
  fetchImpl = fetch,
) {
  ...
  // 2. The current choice (a public read) and the platform's name.
  ...
  let priceId = null;
  if (paid) {
    // 3. The previous price stops being sellable. Paid only: archiving needs
...
function main(argv) {
  ...
  if (!ACCESS_VALUES.includes(access) || (access === "free") !== (usd === undefined)) {
    console.error("usage: node scripts/paywall-setup.mjs free | one_time <usd> | monthly <usd>");
    process.exit(2);
  }
  ...
  setupPaywall(
    ...
    (info) => {
      if (info.access === "free") {
        console.log("paywall: free · self-join open");
        return;
      }
      const price = `$${(Number(info.amount) / 100).toFixed(2)}${info.access === "monthly" ? "/month" : " once"}`;
      console.log(`paywall: ${price} · price ${info.stripe.price_id} · self-join closed`);
    },
    (e) => {
      const detail =
        e instanceof SetupError ? `${e.status} ${JSON.stringify(e.body).slice(0, 300)}` : e.message;
      console.error(`paywall setup failed: ${detail}`);
      process.exit(1);
    },
  );
}
